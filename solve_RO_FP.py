...
#     = gdf.update_data('21April_S15_n8_P59_data.xlsx')
...
for i in range(retailer_number):
    for m in range(product_kind):
        epsilon[i,m]=master.addVar(lb=0,vtype=GRB.CONTINUOUS,name=f'epsilon_{i}_{m+1}')
...
for i in range(retailer_number):
    for m in range(product_kind):
        ...
        master.addConstr(sdim[i,m]<=dim[i][m]*epsilon[i,m])
...
for i in range(retailer_number):
    for m in range(product_kind):
        ...
        check_model.addConstr(wjim_j_sum <= Ui[i+1] * miu_im_check[i, m],name=f'wijm_j_sum<=Ui[i+1]*miu_im_check[{i},{m}]')
...
for i in range(retailer_number):
    ...
    miu_m_sum_M =0
    for m in range(product_kind):
        ksi_m_sum_M+=bigM*ksi_im_check[i,m]
        miu_m_sum_M+=bigM*miu_im_check[i,m]
    check_model.addConstr(zij_j_sum <= ksi_m_sum_M,name='z<=M*ksi_{}'.format(i))
    check_model.addConstr(zji_j_sum <= miu_m_sum_M,name='z<=M*miu_{}'.format(i))
