...
for i in range(retailer_number):
    ...
    # miu_im[i] = {}
    for m in range(product_kind):
        epsilon_im[i][m] = subProblem.addVar(lb=0, ub=3, vtype=GRB.CONTINUOUS, name=f'epsilon_{i + 1}_{m + 1}')
...
for i in range(retailer_number):
    for m in range(product_kind):
        ...
        subProblem.addConstr(Irim[i][m] ==Iim0[i + 1][m] + qimk_sol_k_sum - dim[i][m] * epsilon_im[i][m],
...
""" subProblem.optimize """
subProblem.Params.TimeLimit=1200
        ...
        for m in range(product_kind):
            ...
            master.addConstr(
                Irim_master[iter_cnt, i, m]==
                            Iim0[i + 1][m] + qimk_master_k_sum - dim[i][m] * epsilon_master[iter_cnt, i, m])
        ...
        for new_iter in range(iter_cnt):
            ...
            if master_sub_numerator-beta_master[new_iter]*master_sub_denominator>=delta_a:
                ...
                for i in range(retailer_number):
                    for j in range(retailer_number):
                        if i != j:
                            cij_zij_prosum.addTerms(-beta_master[new_iter] * cij_trans[i][j], zij_master[new_iter, i, j])
                him_wijm_produm = LinExpr()
                for m in range(product_kind):
                    for i in range(retailer_number):
                        for j in range(retailer_number):
                            if i != j:
                                him_wijm_produm.addTerms(-beta_master[new_iter] * him[i][m], wijm_master[iter_cnt, i, j, m])
                master.addConstr(eta <= pim_wijm_prosum + cij_zij_prosum + him_wijm_produm, name=f'sub_eta_maxmin_cons_{new_iter}')
    ...
    for i in range(retailer_number):
        for m in range(product_kind):
            ...
            subProblem.addConstr(
                Irim[i][m] == Iim0[i + 1][m] + qimk_sol_k_sum -dim[i][m] * epsilon_im[i][m],
                name=f'real_inventory_cons_i{i}_m{m + 1}')
            # subProblem.addConstr(
...
check_model.Params.TimeLimit=1200
